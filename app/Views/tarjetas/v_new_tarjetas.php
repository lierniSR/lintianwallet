...
<head>
    ...
    <title data-tr="tituloNuevaCuenta"><?= tr('tituloNuevaCuenta') ?? 'Nueva Cuenta' ?> | Lintian Wallet</title>
...
<body class="relative min-h-screen bg-gradient-to-b from-purple-600 via-purple-700 to-blue-900 pb-12 font-sans selection:bg-[#29C6AD]/30 flex flex-col">
    ...
    <!-- === Formulario de Creación: Alta de nuevas cuentas/tarjetas === -->
        ...
        <div class="w-full max-w-lg bg-white/10 backdrop-blur-xl rounded-3xl border border-white/20 shadow-2xl overflow-hidden transform transition-all">
            <div class="p-8 md:p-10">
                <div class="mb-8 text-center md:text-left">
                    <h1 class="text-3xl font-bold text-white tracking-tight" data-tr="tituloNuevaCuenta"><?= tr('tituloNuevaCuenta') ?? 'Nueva Cuenta' ?></h1>
                    <p class="text-purple-200 font-medium" data-tr="descNuevaCuenta"><?= tr('descNuevaCuenta') ?? 'Configura tu nueva tarjeta de ahorro.' ?></p>
                </div>
                ...
                <div class="space-y-2 group">
                    ...
                    <div class="relative">
                        ...
                        <input type="number" step="0.01" name="saldoTotal" id="saldoTotal" required
                            ...
                            placeholder="<?= tr('placeholderSaldo') ?? '0,00' ?>" data-tr="placeholderSaldo">
                    </div>
                </div>
                ...
                <div class="space-y-2 group">
                    ...
                    <div class="relative">
                        ...
                        <select name="id_categoria" id="id_categoria" required
                            ...
                            <option value="" disabled selected class="bg-purple-800 text-white" data-tr="opcionSeleccionCategoria"><?= tr('opcionSeleccionCategoria') ?? 'Selecciona una categoría' ?></option>
                            <?php foreach ($categorias as $categoria): ?>
                                <option value="<?= $categoria->id ?>" class="bg-purple-800 text-white">
                                    <?= esc($categoria->nombre) ?>
                                </option>
                            <?php endforeach; ?>
                            <option value="otro" class="bg-purple-800 text-white" data-tr="opcionOtro"><?= tr('opcionOtro') ?? 'Otro...' ?></option>
                        </select>
                    ...
                    <div id="wrapper_nueva_categoria" class="hidden mt-3">
                        <div class="relative">
                            ...
                            <input type="text" name="nueva_categoria" id="nueva_categoria"
                                placeholder="<?= tr('placeholderNuevaCategoria') ?? 'Nombre de la nueva categoría...' ?>" data-tr="placeholderNuevaCategoria"
                        ...
                        <p class="text-yellow-300/70 text-xs mt-1.5 ml-1" data-tr="descNuevaCategoria"><?= tr('descNuevaCategoria') ?? 'Se creará automáticamente una nueva categoría con ese nombre.' ?></p>
                    </div>
                </div>
                ...
                <div class="flex flex-col gap-4 pt-4">
                    <button type="submit" data-tr="btnCrearCuenta"
                        ...
                        <?= tr('btnCrearCuenta') ?? 'Crear Cuenta' ?>
                    </button>
